...
#  --------新增的重排代码
def rerank(data, query, data_type):
    def create_combined_str_for_node(item, data_type):
        if data_type == "node":
    ...
    def get_top_k_results(data, query, data_type, reranker, k=10):
        results = []
        for item in data:
            combined_str = create_combined_str_for_node(item, data_type)
            scores = reranker.compute_score([[query, combined_str]], normalize = True)
    ...
    reranker = FlagReranker('D:/LLM_project/LightRAG_learning/reranker/bge-reranker-v2-m3', use_fp16=True)

    top_k_results = get_top_k_results(data, query, data_type, reranker, k=10)
...
async def _get_node_data(
    ...
    data_type = "node"
    #把召回的node_datas再进行重排
    node_datas = rerank(node_datas, query_origin, data_type)
...
async def _get_edge_data(
    ...
    data_type = "edge"
    #把召回的node_datas再进行重排
    edge_datas = rerank(edge_datas, query_origin, data_type)
